...
import {plotExpenses} from './../Expenses/api-expense';
...
export default function ExpensesMonthlyScatter() {
	...
	const [error, setError] = useState('');
		...
		const searchClicked = () => {
			const jwt = auth.isAuthenticated();
			plotExpenses({month: month},{t: jwt.token}).then((data) => {
				if (data.error) {
					setError(data.error);
				} else {
					setPlot(data);
	...
	<Paper className={classes.root} elevation={4}>
		<div className={classes.chartWrapper}>
			<div className={classes.search}>
				<Typography className={classes.text}> Expenses scattered over </Typography>
			...
			<VictoryChart theme={VictoryTheme.material}
				...
				<VictoryScatter style={{
										data: { fill: "#2bbd7e", stroke: "#69f0ae", strokeWidth: 2 },
										labels: { fill: "#2bbd7e", fontSize: 10, padding:8}}}
								bubbleProperty="y"
								maxBubbleSize={15}
								minBubbleSize={5}
				...
				<VictoryLabel textAnchor="middle"
					...
					/>
				<VictoryLabel textAnchor="middle"
								style={{ fontSize: 14, fill: '#8b8b8b', paddingBottom:'10' }}
								x={6} y={190}
								angle = {270}
								text={`Amount ($)`}
					/>
